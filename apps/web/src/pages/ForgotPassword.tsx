    ...
    <div className="min-h-screen flex items-center justify-center bg-[#09090b] px-4 relative">
      ...
      <div className="w-full max-w-[360px] relative z-10">
        ...
        <div className="mb-8">
          ...
          <p className="text-sm text-zinc-500 mt-2">{t('auth.forgotPasswordSubtitle')}</p>
        </div>

        {success ? (
          <div className="p-4 bg-emerald-500/10 border border-emerald-500/15 text-emerald-400 text-sm rounded-lg">
            <p className="font-medium mb-1">{t('auth.resetEmailSent')}</p>
            <p className="text-emerald-400/70 text-xs">{t('auth.resetEmailSentDesc')}</p>
          </div>
        ) : (
          <>
            {error && (
